...
public class AIManager : MonoBehaviour
{
    ...
    public void RunAITurn(ProvinceManagerScript province)
    {
        ...
        List<float> strategicOutputs = currentAI.ProcessStrategicNetwork(strategicInputs,4);
        float unitMode = strategicOutputs[0]; //output 0: strategic mode passed to weight AI
        float constructUnit = strategicOutputs[1]; //output 1: amount of units to recruit
        float constructTower = strategicOutputs[2]; //output 2: amount of towers to construct
        float constructTown = strategicOutputs[3]; //output 3: amount of towns to construct
        ...
        foreach (Vector3Int position in allConnectedPositions)
        {
            ...
            weightInputs.Add((float)((tile.team == province.team) ? 1 : (tile.team == 0) ? 0 : -1)); //input 1: team attribute (1 if friendly, 0 if neutral, -1 if hostile
            ...
            float activationWeight = weightOutputs[0]; //output 0: priority for moving this unit (new spawns are always done last?)
        ...
        //spawn units
        for (float x = 1; x <= constructUnit; x += 1)
        {
            float bestWeight = -9999;
            Vector3Int bestPosition = Vector3Int.zero;
            foreach (Vector3Int position in allConnectedPositions)
